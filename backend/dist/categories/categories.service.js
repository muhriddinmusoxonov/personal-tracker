...
let CategoriesService = class CategoriesService {
    ...
    async ensureDefaultsForUser(userId) {
        const existing = await this.categoryModel.countDocuments({ owner: userId, isDefault: true });
        if (existing > 0)
            return;
        await this.categoryModel.insertMany(DEFAULT_CATEGORIES.map((c) => ({ ...c, owner: userId, isDefault: true })));
    }
    findAllForUser(userId, type) {
